...
$api->get('/pages', function (Request $request) {
    ...
    $count = ServiceKernel::instance()->getBiz()->service('User:UserService')->searchUserCount(array());
    $users = ServiceKernel::instance()->getBiz()->service('User:UserService')->searchUsers(array(), array('createdTime', 'DESC'), $start, $limit);
    return array(
...
$api->post('/login', function (Request $request) {
    ...
    $user = ServiceKernel::instance()->getBiz()->service('User:UserService')->getUserByLoginField($fields['nickname']);

    if (empty($user)) {
        throw new ResourceNotFoundException('User', $fields['nickname']);
    }

    if (!ServiceKernel::instance()->getBiz()->service('User:UserService')->verifyPassword($user['id'], $fields['password'])) {

        throw new RuntimeException('password error');
    }

    $token = ServiceKernel::instance()->getBiz()->service('User:UserService')->makeToken('mobile_login', $user['id']);
    setCurrentUser($user);
    return array(
...
$api->post('/bind_login', function (Request $request) {
    ...
    $userBind = ServiceKernel::instance()->getBiz()->service('User:UserService')->getUserBindByTypeAndFromId($type, $id);
    ...
    if (empty($userBind)) {
        ...
        if (!ServiceKernel::instance()->getBiz()->service('User:AuthService')->isRegisterEnabled()) {
        ...
        $token = ServiceKernel::instance()->getBiz()->service('User:UserService')->makeToken('mobile_login', $user['id'], time() + 3600 * 24 * 30);
        setCurrentUser($user);
        $user = $userUtil->fillUserAttr($user['id'], $oauthUser);
    } else {
        $user = ServiceKernel::instance()->getBiz()->service('User:UserService')->getUser($userBind['toId']);
        $token = ServiceKernel::instance()->getBiz()->service('User:UserService')->makeToken('mobile_login', $user['id'], time() + 3600 * 24 * 30);
        setCurrentUser($user);
    }
...
$api->post('/logout', function (Request $request) {
    ...
    $result = ServiceKernel::instance()->getBiz()->service('User:UserService')->deleteToken('login', $token);
    return array(
...
$api->post('/{id}/followers', function (Request $request, $id) {
    ...
    if (!empty($method) && $method == 'delete') {
        $result = ServiceKernel::instance()->getBiz()->service('User:UserService')->unFollow($fromUser['id'], $id);
    } else {
        $result = ServiceKernel::instance()->getBiz()->service('User:UserService')->follow($fromUser['id'], $id);
    }
...
$api->get('/{id}/friendship', function (Request $request, $id) {
    ...
    foreach ($toIds as $toId) {
        $toUser = ServiceKernel::instance()->getBiz()->service('User:UserService')->getUser($toId);
