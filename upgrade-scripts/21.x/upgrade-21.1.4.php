...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    public function resetCourseTaskCopyId()
    {
        $count = $this->getConnection()->fetchColumn("
            SELECT COUNT(*) FROM course_task ct INNER JOIN course_v8 c ON c.locked = 0 AND c.id = ct.courseId AND ct.copyId > 0;
        ");
        ...
        $this->logger('info', "开始重置{$count}个课时任务的copyId");
        $this->getConnection()->exec("
            UPDATE course_task ct INNER JOIN course_v8 c ON c.locked = 0 AND c.id = ct.courseId AND ct.copyId > 0 SET copyId = 0;
        ");
