...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    public function clearAppUserToken()
    {
        if ($this->isTableExist('user_token') && $this->isTableExist('user')) {
            $delSql = "delete from user_token where userId in (select id from user where verifiedMobile is null or verifiedMobile = '') and type = 'mobile_login' and UNIX_TIMESTAMP(NOW()) + 7*86400 >= expiredTime";
            $this->getConnection()->exec($delSql);
