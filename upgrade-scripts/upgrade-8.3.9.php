...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    public function updateCourseSeq()
    {
        if (!$this->getSettingService()->get('updateCourseSeq')) {
            $courses = $this->getCourseService()->searchCourses(array(), array('seq' => 'DESC', 'createdTime' => 'ASC'), 0, PHP_INT_MAX);
            $groupCourses = ArrayToolkit::group($courses, 'courseSetId');

            foreach ($groupCourses as $key => $courses) {
                $this->getCourseService()->sortCourse($key, ArrayToolkit::column($courses, 'id'));
            }

            $this->logger('info', '教学计划顺序更新');
            $this->getSettingService()->set('updateCourseSeq', array('updatedTime' => time(), 'version' => '8.3.9'));
