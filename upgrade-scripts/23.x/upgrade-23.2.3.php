...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    public function modifyCertificateRecord()
    {
        $this->getConnection()->exec("ALTER TABLE `certificate_record` MODIFY COLUMN `expiryTime` int unsigned NOT NULL DEFAULT '0' COMMENT '过期时间';");

        return 1;
    }

    public function addUserTokenIndex()
    {
        if (!$this->isIndexExist('user_token', 'userid_type_idx')) {
            $this->getConnection()->exec("ALTER TABLE `user_token` ADD INDEX `userid_type_idx` (`userId`, `type`(10));");
        }
