...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    protected function updateCourseTitle($page)
    {
        ...
        $start = $this->getStart($page);
        $sql = "select id from `course_v8` where isDefault = 1 and courseSetId in (SELECT courseSetId FROM course_v8 where isDefault=1 group by courseSetId HAVING count(courseSetId)=1 ) limit {$start}, {$this->pageSize}";
    ...
    protected function updateCourseSetSummary($page)
    {
        ...
        $start = $this->getStart($page);
        $sql = "select id, summary, courseSetId from course_v8 where isDefault = 1 and courseSetId in (SELECT courseSetId FROM course_v8 where isDefault=1 group by courseSetId HAVING count(courseSetId)=1) limit {$start}, {$this->pageSize}";
        $courses = $this->getConnection()->fetchAll($sql);
        foreach ($courses as $course) {
