...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    {
        $count = self::getSchedulerService()->countJobs(array('name' => 'Xapi_PushStatementsJob', 'source' => \Biz\Crontab\SystemCrontabInitializer::SOURCE_SYSTEM));
        if ($count > 0) {
            self::getSchedulerService()->deleteJobByName('Xapi_PushStatementsJob');
        }

        $count = self::getSchedulerService()->countJobs(array('name' => 'Xapi_AddActivityWatchToStatementJob', 'source' => \Biz\Crontab\SystemCrontabInitializer::SOURCE_SYSTEM));
        if ($count > 0) {
            self::getSchedulerService()->deleteJobByName('Xapi_AddActivityWatchToStatementJob');
        }

        $count = self::getSchedulerService()->countJobs(array('name' => 'Xapi_ArchiveStatementJob', 'source' => \Biz\Crontab\SystemCrontabInitializer::SOURCE_SYSTEM));
        if ($count > 0) {
