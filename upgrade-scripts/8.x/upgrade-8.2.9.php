...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    private function updateScheme($index)
    {
        $definedFuncNames = array(
            'addConvertStatusToUploadFiles',
            'refreshLianLianPayRefundTime',
            'resetCrontabJobNum',
        );
    ...
    protected function addConvertStatusToUploadFiles()
    {
        if (!$this->isFieldExist('upload_files', 'audioConvertStatus')) {
            $this->getConnection()->exec("ALTER TABLE `upload_files` ADD `audioConvertStatus` enum('none', 'doing', 'success', 'error') NOT NULL DEFAULT 'none' COMMENT '视频转音频的状态';");
        }

        return 1;
    }

    protected function refreshLianLianPayRefundTime()
    {
        $this->getConnection()->exec("update biz_order set refund_deadline = expired_refund_days * 86400 + updated_time, pay_time = updated_time where payment = 'lianlianpay' and length(pay_time) = 8;");
        return 1;
    }
