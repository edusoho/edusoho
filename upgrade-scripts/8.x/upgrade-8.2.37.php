...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    protected function addStatusIndex()
    {
        if (!$this->isIndexExist('status', 'classroomId', 'classroomId')) {
            if (!$this->isJobExist('updateStatusIndexJob')) {
                //在升级第二天的凌晨2点执行
                $execTime = strtotime(date('Y-m-d', time())) + 86400 + 3600 * 2;
                $jobFields = array(
                    'name' => 'updateStatusIndexJob',
                    'expression' => $execTime,
                    'class' => 'Biz\User\Job\UpdateStatusIndexJob',
            ...
            $this->logger('info', '新增updateStatusIndexJob的定时任务完成,预计第二天凌晨2点执行');
        }
