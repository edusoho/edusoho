...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    protected function execMigrate()
    {
        $connection = $this->getConnection();
        if (!$this->isFieldExist('testpaper_result_v8', 'metas')) {
            $connection->exec("ALTER TABLE `testpaper_result_v8` ADD COLUMN `metas` text COMMENT '练习的题型排序等附属信息' AFTER `updateTime`; ");
        }

        $this->logger('info', '更新字段成功！');
