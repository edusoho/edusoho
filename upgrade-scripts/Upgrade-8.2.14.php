...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    protected function fixOrderStatus($page)
    {
        $limit = 1000;
        $start = ($page - 1) * $limit;
        $allCount = $this->getOrderRefundDao()->count(array());
        if ($start > $allCount) {
            return 1;
        }

        $refunds = $this->getOrderRefundDao()->search(array('status' => 'auditing'), array('id' => 'asc'), $start, $limit);
        if (empty($refunds)) {
            return $page + 1;
        }
        ...
        $sql = "
            update  `biz_order` set `status` = 'success' where id in ({$orderIds});
            update  `biz_order_item` set `status` = 'success' where order_id in ({$orderIds});
        ";
