...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    public function alterTableCourseMaterialAddIndex()
    {
        if (!$this->isIndexExist('classroom_member', 'userId_classroomId')) {
            $this->getConnection()->exec('ALTER TABLE `classroom_member` ADD INDEX `userId_classroomId` (`userId`, `classroomId`);');
        }
        $this->logger('info', '`classroom_member`表添加索引');

        if (!$this->isIndexExist('course_task', 'fromCourseSetId')) {
            $this->getConnection()->exec('ALTER TABLE `course_task` ADD INDEX `fromCourseSetId` (`fromCourseSetId`);');
        }
        $this->logger('info', '`course_task`表添加索引');

        if (!$this->isIndexExist('activity', 'fromCourseId')) {
            $this->getConnection()->exec('ALTER TABLE `activity` ADD INDEX `fromCourseId` (`fromCourseId`);');
        }
        $this->logger('info', '`course_task`表添加索引');
        return 1;
    }
