...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    public function addBizItemCategoryColumn()
    {
        if (!$this->isFieldExist('biz_item_category', 'item_num')) {
            $this->getConnection()->exec("ALTER TABLE `biz_item_category` ADD `item_num` INT(11) NOT NULL DEFAULT '0' COMMENT '题目总数' AFTER `bank_id`;");
        }

        if (!$this->isFieldExist('biz_item_category', 'question_num')) {
            $this->getConnection()->exec("ALTER TABLE `biz_item_category` ADD `question_num` INT(11) NOT NULL DEFAULT '0' COMMENT '问题总数' AFTER `item_num`;");
        }
    ...
    public function createItemBankExerciseMemberOperationRecordTable()
    {
        ...
        if (!$this->isTableExist('item_bank_exercise_member_operation_record')) {
            $this->getConnection()->exec("
                ...
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='题库练习成员操作记录表';
            ");
        }
