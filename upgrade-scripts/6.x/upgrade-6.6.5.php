...
use Symfony\Component\Yaml\Yaml;
 ...
 class EduSohoUpgrade extends AbstractUpdater
 {
    ...
    private function updateScheme()
    {
        ...
        $connection->exec("DELETE FROM `crontab_job` where name='DeleteExpiredTokenJob';");
        $connection->exec("DELETE FROM `crontab_job` where name='CancelOrderJob';");

        $connection->exec("DELETE FROM `crontab_job` where name='DeleteExpiredTokenJob';");
        $connection->exec("DELETE FROM `crontab_job` where name='CancelOrderJob';");

        $connection->exec("INSERT INTO `crontab_job` (`name`, `cycle`, `cycleTime`, `jobClass`, `jobParams`, `executing`, `nextExcutedTime`, `latestExecutedTime`, `creatorId`, `createdTime`) 
            VALUES
        ...
        $connection->exec("DELETE from `log` where message='更新邮件服务器设置';");
