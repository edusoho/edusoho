...
class EduSohoUpgrade extends AbstractUpdater
{
    ...
    {
        $definedFuncNames = [
            'alterTableCourseMaterialAddIndex',
        ];
        $funcNames = array();
    ...
    public function alterTableCourseMaterialAddIndex()
    {
        if (!$this->isIndexExist('course_material_v8', 'fileId_courseSetId')) {
            $this->getConnection()->exec('ALTER TABLE `course_material_v8` ADD INDEX `fileId_courseSetId` (`fileId`, `courseSetId`);');
        }
        if (!$this->isIndexExist('course_material_v8', 'copyId')) {
            $this->getConnection()->exec('ALTER TABLE `course_material_v8` ADD INDEX `copyId` (`copyId`);');
        }
        $this->logger('info', '`course_material_v8`表添加索引');
