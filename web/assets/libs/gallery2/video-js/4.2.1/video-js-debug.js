define("gallery2/video-js/4.2.1/video-js-debug", [], function(require, exports, module) {
    /**
 ...
 */
    ...
    vjs.Slider.prototype.update = function() {
        ...
        if (handle) {
            var box = this.el_, boxWidth = box.offsetWidth, handleWidth = handle.el().offsetWidth, // The width of the handle in percent of the containing box
            // In IE, widths may not be ready yet causing NaN
            handlePercent = handleWidth ? handleWidth / boxWidth : 0, // Get the adjusted size of the box, considering that the handle's center never touches the left or right side.
            // There is a margin of half the handle's width on both sides.
            boxAdjustedPercent = 1 - handlePercent, // Adjust the progress that we'll use to set widths to the new adjusted box width
 ...
 */
    ...
    vjs.Player.prototype.buffered = function() {
        var buffered = this.techGet("buffered"), start = 0, buflast = buffered.length - 1, // Default end to 0 and store in values
    ...
    vjs.Player.prototype.src = function(source) {
        ...
        if (source instanceof Array) {
            var sourceTech = this.selectSource(source), techName;
            if (sourceTech) {
 ...
 */
    ...
    vjs.Html5.prototype.createEl = function() {
        var player = this.player_, // If possible, reuse original tag for HTML5 playback technology element
        el = player.tag, newEl;
 ...
 */
    vjs.Flash = vjs.MediaTechController.extend({
        ...
        init: function(player, options, ready) {
            vjs.MediaTechController.call(this, player, options, ready);
            var source = options["source"], // Which element to embed in
            parentEl = options["parentEl"], // Create a temporary element to be replaced by swf object
            placeHolder = this.el_ = vjs.createEl("div", {
                id: player.id() + "_temp_flash"
            }), // Generate ID for swf object
            objId = player.id() + "_flash_api", // Store player options in local var for optimization
            // TODO: switch to using player methods instead of options
            // e.g. player.autoplay();
            playerOptions = player.options_, // Merge default flashvars with ones passed in to init
            ...
            if (options["iFrameMode"] === true && !vjs.IS_FIREFOX) {
                ...
                vjs.on(iFrm, "load", vjs.bind(this, function() {
                    var iDoc, iWin = iFrm.contentWindow;
 ...
 */
    ...
    vjs.Flash.canPlaySource = function(srcObj) {
        if (srcObj.type in vjs.Flash.formats || srcObj.type in vjs.Flash.streamingFormats) {
            return "maybe";
        }
    };
    vjs.Flash.formats = {
    ...
    vjs.Flash.embed = function(swf, placeHolder, flashVars, params, attributes) {
        var code = vjs.Flash.getEmbedCode(swf, flashVars, params, attributes), // Get element by embedding code and retrieving created element
    ...
    vjs.TextTrack.prototype.update = function() {
        if (this.cues_.length > 0) {
            ...
            if (this.prevChange === undefined || time < this.prevChange || this.nextChange <= time) {
                var cues = this.cues_, // Create a new time box for this state.
                newNextChange = this.player_.duration(), // Start at beginning of the timeline
                newPrevChange = 0, // Start at end
                reverse = false, // Set the direction of the loop through the cues. Optimized the cue check.
                newCues = [], // Store new active cues.
                // Store where in the loop the current active cues are, to provide a smart starting point for the next loop.
                firstActiveIndex, lastActiveIndex, cue, i;
                // Loop vars
 ...
 */
    ...
    videojs.options.flash.swf = require.resolve("./video-js.swf#");
    module.exports = vjs;
});
