...
if ($('.js-homepage').length > 0) {
  ...
  setInterval(function () {
    getSignStatus($('input[name=course-id]', window.parent.document).val(), $('input[name=task-id]', window.parent.document).val());
  }, 1000);

}

function getSignStatus(courseId, taskId)
{
  $.ajax({
    url: 'http://www.esdev.com/course/'+courseId+'/task/'+taskId+'/learning?routeName=status',
    method: 'GET',
