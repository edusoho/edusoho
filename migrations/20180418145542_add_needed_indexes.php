...
class AddNeededIndexes extends Migration
{
    ...
    public function up()
    {
        ...
        if (!$this->isIndexExist('biz_pay_trade', 'type', 'type')) {
            $connection->exec('
                ALTER TABLE `biz_pay_trade` ADD INDEX `type`(`type`);
            ');
        }

        if (!$this->isIndexExist('classroom_member', 'classroomId', 'classroomId')) {
            $connection->exec('
        ...
        if (!$this->isIndexExist('course_member', 'courseSetId', 'courseSetId')) {
            $connection->exec('
        ...
        if (!$this->isIndexExist('course_task_result', 'courseTaskId', 'courseTaskId_activityId')) {
            $connection->exec('
                ALTER TABLE `course_task_result` ADD INDEX `courseTaskId_activityId`(`courseTaskId`,`activityId`);
            ');
        }

        if (!$this->isIndexExist('course_v8', 'courseSetId', 'courseSetId_status')) {
            $connection->exec('
        ...
        if (!$this->isIndexExist('status', 'courseId', 'courseId_createdTime')) {
            $connection->exec('
                ALTER TABLE `status` ADD INDEX courseId_createdTime (`courseId`, `createdTime`);
            ');
        }

        if (!$this->isIndexExist('testpaper_item_result_v8', 'resultId', 'resultId_type')) {
            $connection->exec('
                ALTER TABLE `testpaper_item_result_v8` ADD INDEX `resultId_type`(`resultId`,`type`);
            ');
        }

        if (!$this->isIndexExist('testpaper_item_result_v8', 'testId', 'testId_type')) {
            $connection->exec('
        ...
        if (!$this->isIndexExist('upload_files_share', 'sourceUserId', 'sourceUserId')) {
            $connection->exec('
                ALTER TABLE `upload_files_share` ADD INDEX `sourceUserId`(`sourceUserId`);
            ');
        }

        if (!$this->isIndexExist('upload_files_share', 'targetUserId', 'targetUserId')) {
            $connection->exec('
                ALTER TABLE `upload_files_share` ADD INDEX `targetUserId`(`targetUserId`);
            ');
        }

        if (!$this->isIndexExist('upload_files_share', 'createdTime', 'createdTime')) {
            $connection->exec('
