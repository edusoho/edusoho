...
class CrmAddFields extends Migration
{
    ...
    public function up()
    {
        ...
        $db->exec(" UPDATE `course_member` SET `lastLearnTime` = (SELECT max(startTime) FROM `course_lesson_learn` WHERE course_member.courseId = course_lesson_learn.courseId AND course_member.userId = course_lesson_learn.userId);");

        $db->exec(" UPDATE `classroom_member` SET `lastLearnTime` = (SELECT max(lastLearnTime) FROM `course_member` WHERE classroom_member.classroomId = course_member.classroomId AND classroom_member.userId = course_member.userId AND course_member.joinedType = 'classroom');");

        $db->exec(" UPDATE `classroom_member` SET `learnedNum` = (SELECT sum(learnedNum) FROM `course_member` WHERE classroom_member.classroomId = course_member.classroomId AND classroom_member.userId = course_member.userId AND course_member.joinedType = 'classroom');");
