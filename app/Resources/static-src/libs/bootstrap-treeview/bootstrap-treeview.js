...
;(function ($, window, document, undefined) {
  ...
  */
  ...
  Tree.prototype.clickHandler = function (event) {
    ...
    if ((classList.indexOf('expand-icon') !== -1)) {
      ...
      if (node.selectable && !node.state.disabled) {
        this.$element.trigger('nodeElementSelect', node);
  ...
  Tree.prototype.buildTree = function (nodes, level) {
    ...
    $.each(nodes, function addNodes(id, node) {
      ...
      if (_this.options.showCheckbox && !node.hideCheckbox) {
      ...
      if (_this.options.enableLinks) {
        ...
        treeItem
          .append($(_this.template.link)
            .attr('href', node.href)
            .append($(_this.template.name).text(node.name))
          );
      }
      else {
        // otherwise just text
  ...
  Tree.prototype.buildStyleOverride = function (node) {
    ...
    color = node.state.color ? node.state.color : color;
    backColor = node.state.backColor ? node.state.backColor : backColor;
  ...
  */
  ...
  Tree.prototype.setElementStyle = function () {
...
/**
  2017-06-26
  修改node.text为node.name
*/

/**
  2017-07-04
  添加 Tree.prototype.setElementStyle
*/


/**
 * 2017-08-01
 * 去除默认插入的css Tree.prototype.css
 */


/**
 * 2017-08-01
 * 重新定义disabled的行为，设置select 和check不可用，expand可用
 */
