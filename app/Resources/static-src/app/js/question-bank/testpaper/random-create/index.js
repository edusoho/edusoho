...
class TestpaperForm {
  ...
  _initSelect() {
    $('[name="ranges[categoryId]"]').select2({
      treeview: true,
...
$('[name="ranges[categoryId]"]').change(function(){
  let url = $(this).data('checkNumUrl');
  checkQuestionNum(url);
});

function checkQuestionNum(url) {
  let bankId = $('[name="ranges[categoryId]"]').data('bankId');
  let categoryId = $('[name="ranges[categoryId]"]').val();

  $.post(url,{bankId:bankId, categoryId:categoryId},function(data){
