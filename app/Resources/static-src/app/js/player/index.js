...
class Show {
  ...
  constructor(element) {
    ...
    if ($iframe.length > 0) {
      //播放到最后一秒视为上次播放到0秒
      let lastLearnTime = parseInt($iframe.data('lastLearnTime')) + parseInt($iframe.data('videoHeaderLength'));
      lastLearnTime = (parseInt(this.fileLength) <= lastLearnTime) ? 0 : $iframe.data('lastLearnTime');
