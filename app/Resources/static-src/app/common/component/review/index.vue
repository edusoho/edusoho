  ...
  <div>
    ...
    <div class="reviews">
      <create-review :target-id="targetId" :goods="goods" :target-type="targetType" :can-create="canCreate"
        :current-user-id="currentUserId" :captcha="captcha"></create-review>
      <div class="reviews-item" v-for="review in reviews" :key="review.id" :class="'reviews-item-' + review.id">
        <a target="_blank" :class="'card-' + review.user.uuid">
          <img class="js-user-card reviews-item__img" :src="review.user.avatar.large" :data-user-id="review.user.uuid"
            :data-card-url="`/user/${review.user.uuid}/card/show`" alt="">
        </a>
        <div class="reviews-item__text reviews-text">
          <div class="reviews-text__nickname">
            <a class="link-dark js-user-url" :class="'user-url-' + review.user.uuid" :data-userid="review.user.uuid"
              target="_blank">{{ review.user.nickname }}</a>
          ...
          <div class="reviews-text__content" :id="`review-content-${review.id}`" style="white-space: pre-wrap;">{{
            review.content | removeHtml }}<span v-if="currentUserId > 0 && review.me_report" style="color: red;">(已举报)</span>
          </div>
          <div class="reviews-text__reply">
            <a class="review-text__hover" :id="`js-review-modal-${review.id}`"
              v-if="currentUserId > 0 && review.user.id != currentUserId && !review.me_report" href="#modal"
              data-toggle="modal"
          ...
          <div v-else class="reviews-text__reply-content clearfix" :class="'reviews-text__reply-content-' + review.id">
            <ul class="media-list thread-post-list thread-subpost-list">
              <li class="thread-post media" :class="'thread-subpost-' + post.id" v-for="post in review.posts"
                :key="post.id">
                <div class="media-left">
                  <a target="_blank" :class="'card-' + post.user.id">
                    <img class="avatar-sm js-user-card" :src="post.user.avatar.large" :data-user-id="post.user.id"
                      :data-card-url="`/user/${post.user.uuid}/card/show`" alt="">
                  </a>
                </div>
                <div class="media-body" style="overflow: visible !important;">
                  <div class="metas">
                    <div v-if="canOperate || (currentUserId > 0 && post.user.id != currentUserId && !post.me_report)"
                      class="thread-post-manage-dropdown dropdown pull-right">
                      <a href="javascript:;" class="dropdown-toggle color-gray" data-toggle="dropdown">
                        <span class="glyphicon glyphicon-collapse-down"></span>
                      </a>
                      <ul class="dropdown-menu">
                        <li v-if="canOperate">
                          <a href="javascript:" class="js-delete-post" :data-target="'thread-subpost-' + post.id"
                            :data-target-id="post.id">{{ 'site.delete' | trans }}</a>
                        </li>
                        <li v-if="currentUserId > 0 && post.user.id != currentUserId && !post.me_report"
                          :id="`js-review-reply-modal-${post.id}`">
                          <a href="#modal" class="js-report-post" data-toggle="modal"
                            :data-url="`/common/report/${replyReportType}/target_id/${post.id}/tags_modal?contentTarget=review-reply-content-${post.id}&modalTarget=js-review-reply-modal-${post.id}`">举报</a>
                        </li>
                      </ul>
                    </div>

                    <a class="link-dark js-user-url" :class="'user-url-' + post.user.id" :data-userid="post.user.id"
                      target="_blank">
                      {{ post.user.nickname }}
                    </a>
                  ...
                  <div class="editor-text" :id="`review-reply-content-${post.id}`">{{ post.content | removeHtml }} <span
                      v-if="currentUserId > 0 && post.me_report" style="color: red;">(已举报)</span> </div>
                  <div class="ptm pbl"></div>
                </div>
              </li>
            </ul>
            <a href="javascript:;" class="btn btn-default btn-xs pull-right mbs" v-if="canCreate"
              :data-toggle="'reviews-text__reply-content-form-' + review.id" @click="onFormDisplay">{{
                'thread.post.reply' | trans }}</a>
      ...
      <div class="learn-more"><a href="javascript:;"
          ...
          @click="searchReviews(parseInt(paging.offset) + parseInt(paging.limit), paging.limit)" :data-page="multiOffset"
          :data-limit="paging.limit" :data.total="paging.total">查看更多<i class="es-icon es-icon-chevronright"></i></a>
      </div>
    </div>
  </div>
</template>
