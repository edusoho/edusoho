    ...
    return (
      <div className="form-group">
        ...
        <div className="col-sm-8 controls">
          ...
          <div className="mtm">
            <label>
              <input type={type} name='answer[]' data-option-id={this.state.datas.optionId} value={this.props.index}  checked={this.state.datas.checked} className="answer-checkbox" onChange = {(event)=>this.onChangeChecked(event)}/>正确答案 
            </label>
          </div>
          <p className={showDanger ? 'color-danger' : 'hidden'}>请输入选项内容</p>
        </div>
        <div className="col-sm-2">
          <a className="btn btn-default btn-sm" data-option-id={ this.state.datas.optionId }  onClick={(event)=>this.deleteOption(event)} href="javascript:;"><i className="glyphicon glyphicon-trash"></i></a>
        </div>
      </div>
    )
  }
