...
trait ContractDisplayTrait
{
    ...
    private function getContractDetail($contract, $goodsKey)
    {
        ...
        return $this->getHtml($content, $contract);
    }
    ...
    private function getDetailContent($content, $goodsKey)
    {
        ...
        return str_replace(
            ...
            [$userProfile['truename'] ?? '', $user['nickname'] ?? '', $userProfile['idcard'] ?? '', $product['title'] ?? '', $this->getContractService()->generateContractCode(), date('Y年m月d日') ?? '', $order['pay_amount'] ?? ''],
            $content
        );
    }

    protected function getHtml($content, $contract)
    {
