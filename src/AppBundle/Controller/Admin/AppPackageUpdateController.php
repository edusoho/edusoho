...
class AppPackageUpdateController extends BaseController
{
    ...
    public function modalAction(Request $request, $id)
    {
        ...
        return $this->render('admin/app-package-update/modal.html.twig', [
            'package' => $package,
        ]);
    }
    ...
    public function checkEnvironmentAction(Request $request, $id)
    {
        ...
        if (empty($info['accessKey']) || empty($settings['cloud_access_key']) || empty($settings['cloud_secret_key'])) {
            $errors = [sprintf('您尚未申请云平台授权码，<a href="%s">请先申请授权码</a>。', $this->generateUrl('admin_setting_cloud_key_update'))];
        } else {
    ...
    public function checkNewestAction($code)
    {
        try {
            ...
            $result = [
                ...
                'urls' => [
                    'checkDependsUrl' => $this->generateUrl('admin_app_package_update_check_depends', ['id' => $apps[$code]['package']['id']]),
                    'backupFileUrl' => $this->generateUrl('admin_app_package_update_backup_file', ['id' => $apps[$code]['package']['id']]),
                    'backupDbUrl' => $this->generateUrl('admin_app_package_update_backup_db', ['id' => $apps[$code]['package']['id']]),
                    'checkDownloadExtractUrl' => $this->generateUrl('admin_app_package_update_check_download_and_extract', ['id' => $apps[$code]['package']['id']]),
                    'downloadExtractUrl' => $this->generateUrl('admin_app_package_update_download_and_extract', ['id' => $apps[$code]['package']['id']]),
                    'beginUpgradeUrl' => $this->generateUrl('admin_app_package_update_begin_upgrade', ['id' => $apps[$code]['package']['id'], 'type' => 'upgrade']),
                    'checkNewestUrl' => $this->generateUrl('admin_app_package_update_check_newest', ['code' => $code]),
                    'checkEnvironmentUrl' => $this->generateUrl('admin_app_package_update_check_environment', ['id' => $apps[$code]['package']['id']]),
                ],
            ];
        } catch (\Exception $e) {
