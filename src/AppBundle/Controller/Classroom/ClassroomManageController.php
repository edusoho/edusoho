...
class ClassroomManageController extends BaseController
{
    ...
    public function exitRecordExportDatumAction(Request $request, $id)
    {
        list($start, $limit, $exportAllowCount) = ExportHelp::getMagicExportSetting($request);

        list($title, $records, $classroomMemberCount) = $this->getExportData(
            $id,
            $start,
            $limit,
            $exportAllowCount
        ...
        $file = '';
        if (0 == $start) {
            $file = ExportHelp::addFileTitle($request, 'classroom_students_exit_record', $title);
        }
    ...
    public function exitRecordExportCsvAction(Request $request, $id)
    {
        $fileName = sprintf('classroom-%s-%s-(%s).csv', $id, 'exit_record', date('Y-n-d'));

        return ExportHelp::exportCsv($request, $fileName);
    }

    private function getExportData($id, $start, $limit, $exportAllowCount)
    {
        $this->getClassroomService()->tryManageClassroom($id);
        $classroom = $this->getClassroomService()->getClassroom($id);
