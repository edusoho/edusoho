	...
	init(){
        ...
        intervalId = setInterval(function(){
			if(tryCount > 10) {
		        ...
		        let html = Translator.trans('进入直播教室错误，请联系管理员，')
		        ...
		        success: function(data){
		            if(data.error){
		                ...
		                let html = data.error
		                	...
		                	+ "<a href='javascript:document.location.reload()'>"
		                	+ Translator.trans('重试') 
		                	+ "</a>或<a href='javascript:window.close();'>" 
		                	+ Translator.trans('关闭')
		                	+ "</a>";
		                $("#entry").html(html);
		                return;
		            ...
		            if(data.roomUrl) {
		                ...
		                let html = '<iframe name="classroom" src="'
		                	+ data.roomUrl 
		                	+ '" style="position:absolute; left:0; top:0; height:100%; width:100%; border:0px;" scrolling="no"></iframe>';
		                $("body").html(html);
		            }
		            tryCount ++;
		        },
