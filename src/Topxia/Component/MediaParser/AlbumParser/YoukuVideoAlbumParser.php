...
class YoukuVideoAlbumParser extends AbstractAlbumParser
{
	...
	public function parse($url)
	{
		...
		if ($response['code'] != 200) {
			throw $this->createParseException($this->getServiceKernel()->trans('获取优酷视频专辑(%url%)页面内容失败！', array('%url%' =>$url )));
		}
    ...
    private function parseId($url)
    {
        ...
        if (empty($matched)) {
            throw $this->createParseException($this->getServiceKernel()->trans('获取优酷视频专辑ID失败'));
        }
        return $matches[1];
    }
    ...
    private function parseTitle($content)
    {
        ...
        if (empty($matched)) {
            throw $this->createParseException($this->getServiceKernel()->trans('获取优酷视频专辑(%url%)标题失败！', array('%url%' =>$url )));
        }
        return $matches[1];
    }
    ...
    private function parseNumber($content)
    {
        ...
        if (empty($matched)) {
            throw $this->createParseException($this->getServiceKernel()->trans('获取优酷视频专辑(%url%)视频数量失败！', array('%url%' =>$url )));
        }
        return $matches[1];
    }
    ...
    private function parseItems($album)
    {
        ...
        foreach (range(1, ceil($album['number'] / 50)) as $page) {
            ...
            $response = $this->fetchUrl($url);
            if ($response['code'] != 200) {
                throw $this->createParseException($this->getServiceKernel()->trans('获取优酷视频专辑(%url%)视频条目失败！', array('%url%' =>$url )));
            }
            ...
            if (empty($matched)) {
                throw $this->createParseException($this->getServiceKernel()->trans('解析优酷视频专辑(%url%)视频条目失败！', array('%url%' =>$url )));
            }
