...
class SinaOpenCourseAlbumParser extends AbstractAlbumParser
{
	...
	public function parse($url)
	{
		...
		if ($response['code'] != 200) {
            throw $this->createParseException($this->getServiceKernel()->trans('获取网易公开课专辑(%url%)页面内容失败！', array('%url%' =>$rurl )));
        }
    ...
    private function parseTitle($content)
    {
        ...
        if (empty($matched)) {
            throw $this->createParseException($this->getServiceKernel()->trans('获取新浪公开课专辑标题失败'));
        }
        return $matches[1];
    }
    ...
    private function parseSummary($content)
    {
        ...
        if (empty($matched)) {
            throw $this->createParseException($this->getServiceKernel()->trans('获取新浪公开课专辑摘要失败'));
        }
        return $matches[1];
    }
    ...
    private function parseItems($url, $content)
    {
        ...
        if (empty($matched)) {
            throw $this->createParseException($this->getServiceKernel()->trans('获取新浪公开课视频条目信息失败'));
        }
