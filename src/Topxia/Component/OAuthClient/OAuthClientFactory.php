...
class OAuthClientFactory
{
    ...
    public static function create($type, array $config)
    {
    	if (!array_key_exists('key', $config) || !array_key_exists('secret', $config)) {
    		throw new InvalidArgumentException($this->getServiceKernel()->trans('参数$config中，必需包含key, secret两个为key的值'));
    	}

        $clients = self::clients();

        if (!array_key_exists($type, $clients)) {
            throw new InvalidArgumentException($this->getServiceKernel()->trans('参数%type%不正确', array('%type%' =>$type )));
        }
    ...
    public static function clients()
    {
        $clients = array(
            'weibo' => array(
                'name' => $this->getServiceKernel()->trans('微博帐号'),
                'admin_name' => $this->getServiceKernel()->trans('微博登录接口'),
            ...
            'qq' => array(
                'name' => $this->getServiceKernel()->trans('QQ帐号'),
                'admin_name' => $this->getServiceKernel()->trans('QQ登录接口'),
            ...
            'renren' => array(
                'name' => $this->getServiceKernel()->trans('人人帐号'),
                'admin_name' => $this->getServiceKernel()->trans('人人登录接口'),
            ...
            'weixinweb' => array(
                'name' => $this->getServiceKernel()->trans('微信网页登录接口'),
                'admin_name' => $this->getServiceKernel()->trans('微信网页登录接口'),
            ...
            'weixinmob' => array(
                'name' => $this->getServiceKernel()->trans('微信内分享登录接口'),
                'admin_name' => $this->getServiceKernel()->trans('微信内分享登录接口'),
