...
class UserSettingController extends BaseController
{
    public function authAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $this->getLogService()->info('system', 'update_settings', $this->getServiceKernel()->trans('更新注册设置'), $auth);
            $this->setFlashMessage('success', $this->getServiceKernel()->trans('注册设置已保存！'));
        }
    ...
    public function userFieldsAction(Request $request)
    {
        ...
        for ($i = 0; $i < count($fields); $i++) {
            if (strstr($fields[$i]['fieldName'], "textField")) {
                $fields[$i]['fieldName'] = $this->getServiceKernel()->trans('多行文本');
            }

            if (strstr($fields[$i]['fieldName'], "varcharField")) {
                $fields[$i]['fieldName'] = $this->getServiceKernel()->trans('文本');
            }

            if (strstr($fields[$i]['fieldName'], "intField")) {
                $fields[$i]['fieldName'] = $this->getServiceKernel()->trans('整数');
            }

            if (strstr($fields[$i]['fieldName'], "floatField")) {
                $fields[$i]['fieldName'] = $this->getServiceKernel()->trans('小数');
            }

            if (strstr($fields[$i]['fieldName'], "dateField")) {
                $fields[$i]['fieldName'] = $this->getServiceKernel()->trans('日期');
            }
        }
    ...
    public function editUserFieldsAction(Request $request, $id)
    {
        ...
        if (strstr($field['fieldName'], "textField")) {
            $field['fieldName'] = $this->getServiceKernel()->trans('多行文本');
        }

        if (strstr($field['fieldName'], "varcharField")) {
            $field['fieldName'] = $this->getServiceKernel()->trans('文本');
        }

        if (strstr($field['fieldName'], "intField")) {
            $field['fieldName'] = $this->getServiceKernel()->trans('整数');
        }

        if (strstr($field['fieldName'], "floatField")) {
            $field['fieldName'] = $this->getServiceKernel()->trans('小数');
        }

        if (strstr($field['fieldName'], "dateField")) {
            $field['fieldName'] = $this->getServiceKernel()->trans('日期');
        }
    ...
    public function addUserFieldsAction(Request $request)
    {
        ...
        if (isset($field['field_title'])
            && in_array($field['field_title'], array($this->getServiceKernel()->trans('真实姓名'), $this->getServiceKernel()->trans('手机号码'), $this->getServiceKernel()->trans('QQ'), $this->getServiceKernel()->trans('所在公司'), $this->getServiceKernel()->trans('身份证号码'), $this->getServiceKernel()->trans('性别'), $this->getServiceKernel()->trans('职业'), $this->getServiceKernel()->trans('微博'), $this->getServiceKernel()->trans('微信')))) {
            throw $this->createAccessDeniedException($this->getServiceKernel()->trans('请勿添加与默认字段相同的自定义字段！'));
        }
        ...
        if ($field == false) {
            $this->setFlashMessage('danger', $this->getServiceKernel()->trans('已经没有可以添加的字段了!'));
        }
