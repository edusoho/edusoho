    ...
    public function giveCoinAction(Request $request)
    {
        if ($request->getMethod() == "POST") {
            ...
            if ($fields['type'] == "add") {
                ...
                $this->getLogService()->info('cash', 'add_coin', $this->getServiceKernel()->trans('添加 ').$user['nickname'].$this->getServiceKernel()->trans(' %amount% 虚拟币',array( '%amount%'=>$fields['amount'])), array());
            } else { 
                $this->getCashAccountService()->waveDownCashField($account["id"], $fields['amount']);
                $this->getLogService()->info('cash', 'add_coin', $this->getServiceKernel()->trans('扣除 ').$user['nickname'].$this->getServiceKernel()->trans(' %amount% 虚拟币',array('%amount%'=>$fields['amount'])), array());
            }
        }
