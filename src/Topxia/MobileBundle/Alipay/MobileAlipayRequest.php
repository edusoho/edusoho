...
class MobileAlipayRequest
{
    ...
    public function getRequestForm($name)
    {
        ...
        //服务器异步通知页面路径
        $notify_url = $this->request->getSchemeAndHttpHost(). "/mapi/alipay/alipay_notify";
        //需http://格式的完整路径，不允许加?id=123这类自定义参数

        //页面跳转同步通知页面路径
        $call_back_url = $this->request->getSchemeAndHttpHost() . "/mapi/alipay/alipay_callback";
        //需http://格式的完整路径，不允许加?id=123这类自定义参数

        //操作中断返回地址
        $merchant_url = $this->request->getSchemeAndHttpHost() . "/mapi/alipay/alipay_merchant";
