...
class AppServiceImpl extends BaseService implements AppService
{
    ...
    public function registerApp($app)
    {
        if (!ArrayToolkit::requireds($app, array('code', 'name', 'version'))) {
            throw $this->createServiceException($this->getServiceKernel()->trans('参数缺失,注册APP失败!'));
        }
        ...
        $app['developerId']   = 0;
        $app['developerName'] = empty($app['author']) ? $this->getServiceKernel()->trans('未知') : $app['author'];
    ...
    public function hasLastErrorForPackageUpdate($packageId)
    {
        ...
        if (empty($package)) {
            throw $this->createServiceException($this->getServiceKernel()->trans('获取应用包#%packageId%信息失败', array('%packageId%' =>$packageId )));
        }
    ...
    public function checkEnvironmentForPackageUpdate($packageId)
    {
        ...
        if (!class_exists('ZipArchive')) {
            $errors[] = $this->getServiceKernel()->trans('php_zip扩展未激活');
        }

        if (!function_exists('curl_init')) {
            $errors[] = $this->getServiceKernel()->trans('php_curl扩展未激活');
        }
        ...
        if ($filesystem->exists($downloadDirectory)) {
            if (!is_writeable($downloadDirectory)) {
                $errors[] = $this->getServiceKernel()->trans('下载目录(%downloadDirectory%)无写权限', array('%downloadDirectory%' =>$downloadDirectory ));
            }
        } else {
            try {
                $filesystem->mkdir($downloadDirectory);
            } catch (\Exception $e) {
                $errors[] = $this->getServiceKernel()->trans('下载目录(%downloadDirectory%)创建失败', array('%downloadDirectory%' =>$downloadDirectory ));
            }
        }
        ...
        if ($filesystem->exists($backupdDirectory)) {
            if (!is_writeable($backupdDirectory)) {
                $errors[] = $this->getServiceKernel()->trans('备份(%backupdDirectory%)无写权限', array('%backupdDirectory%' =>$backupdDirectory ));
            }
        } else {
            try {
                $filesystem->mkdir($backupdDirectory);
            } catch (\Exception $e) {
                $errors[] = $this->getServiceKernel()->trans('备份(%backupdDirectory%)创建失败', array('%backupdDirectory%' =>$backupdDirectory ));
            }
        }
        ...
        if (!is_writeable("{$rootDirectory}/app")) {
            $errors[] = $this->getServiceKernel()->trans('app目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/src")) {
            $errors[] = $this->getServiceKernel()->trans('src目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/vendor2")) {
            $errors[] = $this->getServiceKernel()->trans('vendor2目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/plugins")) {
            $errors[] = $this->getServiceKernel()->trans('plugins目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/web")) {
            $errors[] = $this->getServiceKernel()->trans('web目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/app/cache")) {
            $errors[] = $this->getServiceKernel()->trans('app/cache目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/app/data")) {
            $errors[] = $this->getServiceKernel()->trans('app/data目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/app/config")) {
            $errors[] = $this->getServiceKernel()->trans('app/config目录无写权限');
        }

        if (!is_writeable("{$rootDirectory}/app/config/config.yml")) {
            $errors[] = $this->getServiceKernel()->trans('app/config/config.yml文件无写权限');
        }

        $package = $this->getCenterPackageInfo($packageId);

        $this->_submitRunLogForPackageUpdate($this->getServiceKernel()->trans('检查环境'), $package, $errors);
    ...
    public function checkDependsForPackageUpdate($packageId)
    {
        ...
        try {
            ...
            if (!version_compare(System::VERSION, $package['edusohoMinVersion'], '>=')) {
                $errors[] = $this->getServiceKernel()->trans('EduSoho版本需大于等于%packageEdusohoMinVersion%，您的版本为', array('%packageEdusohoMinVersion%' =>$package['edusohoMinVersion'] )).System::VERSION.$this->getServiceKernel()->trans('，请先升级EduSoho');
            }
        } catch (\Exception $e) {
            $errors[] = $e->getMessage();
        }

        $this->_submitRunLogForPackageUpdate($this->getServiceKernel()->trans('检查依赖'), $package, $errors);
    ...
    public function backupDbForPackageUpdate($packageId)
    {
        ...
        try {
            ...
            if (empty($package)) {
                $errors[] = $this->getServiceKernel()->trans('获取应用包#%packageId%信息失败', array('%packageId%' =>$packageId ));
                goto last;
            }
        ...
        last:
        $this->_submitRunLogForPackageUpdate($this->getServiceKernel()->trans('备份数据库'), $package, $errors);
        return $errors;
    }
    ...
    public function backupFileForPackageUpdate($packageId)
    {
        ...
        last:
        $this->_submitRunLogForPackageUpdate($this->getServiceKernel()->trans('备份文件'), $package, $errors);
        return $errors;
    }
    ...
    public function downloadPackageForUpdate($packageId)
    {
        ...
        try {
            ...
            if (empty($package)) {
                throw $this->createServiceException($this->getServiceKernel()->trans('应用包#%packageId%不存在或网络超时，读取包信息失败', array('%packageId%' =>$packageId )));
            }
        ...
        $this->_submitRunLogForPackageUpdate($this->getServiceKernel()->trans('下载应用包'), $package, $errors);
        return $errors;
    }
    ...
    public function beginPackageUpdate($packageId, $type, $index = 0)
    {
        ...
        if (empty($index)) {
            try {
                ...
                $errors[] = $this->getServiceKernel()->trans('删除文件时发生了错误：%getMessage%', array('%getMessage%' =>$e->getMessage() ));
            ...
            try {
                ...
                $errors[] = $this->getServiceKernel()->trans('复制升级文件时发生了错误：%getMessage%', array('%getMessage%' =>$e->getMessage() ));
                $this->createPackageUpdateLog($package, 'ROLLBACK', implode('\n', $errors));
                goto last;
            }
        }
        ...
        try {
            ...
            $errors[] = $this->getServiceKernel()->trans('执行升级/安装脚本时发生了错误：%getMessage%', array('%getMessage%' =>$e->getMessage() ));
            $this->createPackageUpdateLog($package, 'ROLLBACK', implode('\n', $errors));
            goto last;
        }
        ...
        try {
            ...
            $errors[] = $this->getServiceKernel()->trans('应用安装升级成功，但刷新缓存失败！请检查%cachePath%的权限', array('%cachePath%' =>$cachePath ));
            $this->createPackageUpdateLog($package, 'ROLLBACK', implode('\n', $errors));
            goto last;
        }
        ...
        last:
        $this->_submitRunLogForPackageUpdate($this->getServiceKernel()->trans('执行升级'), $package, $errors);
    ...
    public function updateAppVersion($id, $version)
    {
        ...
        if (empty($app)) {
            throw $this->createServiceException($this->getServiceKernel()->trans('App #%id%不存在，更新版本失败！', array('%id%' =>$id )));
        }

        $this->getLogService()->info('system', 'update_app_version', $this->getServiceKernel()->trans('强制更新应用「%appName%」版本为「%version%」', array('%appName%' =>$app['name'], '%version%' =>$version )));
    ...
    protected function _submitRunLogForPackageUpdate($message, $package, $errors)
    {
        $this->createAppClient()->submitRunLog(array(
            ...
            'message'     => $message.(empty($errors) ? $this->getServiceKernel()->trans('成功') : $this->getServiceKernel()->trans('失败')),
    ...
    protected function unzipPackageFile($filepath, $unzipDir)
    {
        ...
        if ($zip->open($filepath) === true) {
            ...
            throw new \Exception($this->getServiceKernel()->trans('无法解压缩安装包！'));
        }
    }
    ...
    protected function addEduSohoMainApp()
    {
        $app = array(
            'code'          => 'MAIN',
            'name'          => $this->getServiceKernel()->trans('EduSoho主系统'),
            'description'   => $this->getServiceKernel()->trans('EduSoho主系统'),
            'icon'          => '',
            'version'       => System::VERSION,
            'fromVersion'   => '0.0.0',
            'developerId'   => 1,
            'developerName' => $this->getServiceKernel()->trans('EduSoho官方'),
            'installedTime' => time(),
            'updatedTime'   => time()
        );
