...
class CashOrdersServiceImpl extends BaseService implements CashOrdersService
{
    ...
    public function addOrder($order)
    {
        ...
        if (!is_numeric($order['amount'])) {
            throw $this->createServiceException($this->getServiceKernel()->trans('充值金额必须为整数!'));
        }

        $coin                 = $coinSetting['cash_rate'] * $order['amount'];
        $order['sn']          = "O".date('YmdHis').rand(10000, 99999);
        $order['status']      = "created";
        $order['title']       = $this->getServiceKernel()->trans('充值购买').$coin.$coinSetting['coin_name'];
    ...
    public function cancelOrder($id, $message = '', $data = array())
    {
        ...
        if (empty($order)) {
            throw $this->createServiceException($this->getServiceKernel()->trans('订单不存在，取消订单失败！'));
        }

        if (!in_array($order['status'], array('created'))) {
            throw $this->createServiceException($this->getServiceKernel()->trans('当前订单状态不能取消订单！'));
        }
    ...
    public function payOrder($payData)
    {
        ...
        try {
            ...
            if (empty($order)) {
                throw $this->createServiceException($this->getServiceKernel()->trans('订单(%payData%)已被删除，支付失败。', array('%payData%' =>$payData['sn'] )));
            }
            ...
            if ($payData['status'] == 'success') {
                ...
                if (intval($payData['amount'] * 100) !== intval($order['amount'] * 100)) {
                    $message = sprintf($this->getServiceKernel()->trans('订单(%s)的金额(%s)与实际支付的金额(%s)不一致，支付失败。'), $order['sn'], $order['amount'], $payData['amount']);
                ...
                if ($this->canOrderPay($order)) {
                    ...
                    $this->_createLog($order['id'], 'pay_success', $this->getServiceKernel()->trans('付款成功'), $payData);

                    $userId = $order["userId"];
                    $inFlow = array(
                        'userId'   => $userId,
                        'amount'   => $order["amount"],
                        'name'     => $this->getServiceKernel()->trans('入账'),
                        'orderSn'  => $order['sn'],
                        'category' => 'outflow',
                        'note'     => '',
                    ...
                    $rmbOutFlow = array(
                        'userId'   => $userId,
                        'amount'   => $order["amount"],
                        'name'     => $this->getServiceKernel()->trans('出账'),
                        'orderSn'  => $order['sn'],
                        'category' => 'inflow',
                        'note'     => '',
                    ...
                    $this->_createLog($order['id'], 'pay_ignore', $this->getServiceKernel()->trans('订单已处理'), $payData);
                }
            } else {
    ...
    public function createPayRecord($id, array $payData)
    {
        ...
        $this->_createLog($order['id'], 'cash_pay_create', $this->getServiceKernel()->trans('创建交易'), $payData);
    }
