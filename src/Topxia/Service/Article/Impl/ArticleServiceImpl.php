...
class ArticleServiceImpl extends BaseService implements ArticleService
{
    ...
    public function getArticlePrevious($currentArticleId)
    {
        ...
        if (empty($article)) {
            $this->createServiceException($this->getServiceKernel()->trans('文章内容为空,操作失败！'));
        }
        ...
        if (empty($category)) {
            $this->createServiceException($this->getServiceKernel()->trans('文章分类不存在,操作失败！'));
        }
    ...
    {
        if (empty($article)) {
            throw $this->createServiceException($this->getServiceKernel()->trans('文章内容为空，创建文章失败！'));
        }

        $article = $this->filterArticleFields($article, 'add');
        $article = $this->getArticleDao()->addArticle($article);

        $this->getLogService()->info('article', 'create', $this->getServiceKernel()->trans('创建文章《(%articleTitle%)》(%articleId%)', array('%articleTitle%' =>$article['title'], '%articleId%' =>$article['id'] )));
    ...
    public function updateArticle($id, $article)
    {
        ...
        if (empty($checkArticle)) {
            throw $this->createServiceException($this->getServiceKernel()->trans('文章不存在，操作失败。'));
        }
        ...
        $this->getLogService()->info('Article', 'update', $this->getServiceKernel()->trans('修改文章《(%articleTitle%)》(%articleId%)', array('%articleTitle%' =>$article['title'], '%articleId%' =>$article['id'] )));
    ...
    public function like($articleId)
    {
        ...
        if (empty($user)) {
            throw $this->createNotFoundException($this->getServiceKernel()->trans('用户还未登录,不能点赞。'));
        }

        $article = $this->getArticle($articleId);

        if (empty($article)) {
            throw $this->createNotFoundException($this->getServiceKernel()->trans('资讯不存在，或已删除。'));
        }

        $like = $this->getArticleLike($articleId, $user['id']);

        if (!empty($like)) {
            throw $this->createAccessDeniedException($this->getServiceKernel()->trans('不可重复对一条资讯点赞！'));
        }
    ...
    public function setArticleProperty($id, $property)
    {
        ...
        if (empty($property)) {
            throw $this->createServiceException($this->getServiceKernel()->trans('属性%property%不存在，更新失败！', array('%property%' =>$property )));
        }

        $propertyVal = 1;
        $this->getArticleDao()->updateArticle($id, array("{$property}" => $propertyVal));

        $this->getLogService()->info('setArticleProperty', 'updateArticleProperty', $this->getServiceKernel()->trans('文章#%id%,%articleProperty%=>%propertyVal%', array('%id%' =>$id, '%articleProperty%' =>$article[$property], '%propertyVal%' =>$propertyVal )));
    ...
    public function cancelArticleProperty($id, $property)
    {
        ...
        $this->getLogService()->info('cancelArticleProperty', 'updateArticleProperty', $this->getServiceKernel()->trans('文章#%id%,%articleProperty%=>%propertyVal%', array('%id%' =>$id, '%articleProperty%' =>$article[$property], '%propertyVal%' =>$propertyVal )));
    ...
    public function trashArticle($id)
    {
        ...
        $this->getLogService()->info('Article', 'trash', $this->getServiceKernel()->trans('文章#%id%移动到回收站', array('%id%' =>$id )));
    }
    ...
    public function deleteArticle($id)
    {
        ...
        $this->getLogService()->info('Article', 'delete', $this->getServiceKernel()->trans('文章#%id%永久删除', array('%id%' =>$id )));
    ...
    public function publishArticle($id)
    {
        ...
        $this->getLogService()->info('Article', 'publish', $this->getServiceKernel()->trans('文章#%id%发布', array('%id%' =>$id )));
    }

    public function unpublishArticle($id)
    {
        $this->getArticleDao()->updateArticle($id, $fields = array('status' => 'unpublished'));
        $this->getLogService()->info('Article', 'unpublish', $this->getServiceKernel()->trans('文章#%id%未发布', array('%id%' =>$id )));
    }
