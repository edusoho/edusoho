...
class MoneyCardBatchDaoImpl extends BaseDao {
	...
	public function findBatchByCardPrefix($cardPrefix) {

        $sql = "SELECT * FROM {$this->table} WHERE cardPrefix = ? LIMIT 1";

        return $this->getConnection()->fetchAssoc($sql, array($cardPrefix));
    }
