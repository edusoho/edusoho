...
class ContentServiceImpl extends BaseService implements ContentService
{
	...
	public function createContent($content)
	{
		if (empty($content['type'])) {
			throw $this->createServiceException($this->getServiceKernel()->trans('参数缺失，创建内容失败！'));
		}
		...
		if (empty($content['title'])) {
			throw $this->createServiceException($this->getServiceKernel()->trans('内容标题不能为空，创建内容失败！'));
		}
  ...
  //       }
        ...
        $this->getLogService()->info('content', 'create', $this->getServiceKernel()->trans('创建内容《(%contentTitle%)》(%contentId%)', array('%contentTitle%' =>$content['title'], '%contentId%' =>$content['id'] )), $content);
	...
	public function updateContent($id, $fields)
	{
		$content = $this->getContent($id);
		if (empty($content)) {
			throw $this->createServiceException($this->getServiceKernel()->trans('内容不存在，更新失败！'));
		}
		...
		$this->getLogService()->info('content', 'update', $this->getServiceKernel()->trans('内容《(%contentTitle%)》(%contentId%)更新', array('%contentTitle%' =>$content['title'], '%contentId%' =>$content['id'] )), $content);
	...
	public function trashContent($id)
	{
		...
		$this->getLogService()->info('content', 'trash', $this->getServiceKernel()->trans('内容#%id%移动到回收站', array('%id%' =>$id )));
	}

	public function deleteContent($id)
	{
		$this->getContentDao()->deleteContent($id);
		$this->getLogService()->info('content', 'delete', $this->getServiceKernel()->trans('内容#%id%永久删除', array('%id%' =>$id )));
	}

	public function publishContent($id)
	{
		$this->getContentDao()->updateContent($id, $fields = array('status' => 'published'));
		$this->getLogService()->info('content', 'publish', $this->getServiceKernel()->trans('内容#%id%发布', array('%id%' =>$id )));
	}
