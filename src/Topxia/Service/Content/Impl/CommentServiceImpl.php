...
class CommentServiceImpl extends BaseService implements CommentService
{
	...
	public function deleteComment($id)
	{
		...
		if (empty($comment)) {
			throw $this->createNotFoundException($this->getServiceKernel()->trans('评论不存在'));
		}

		if (empty($user)) {
			throw $this->createAccessDeniedException($this->getServiceKernel()->trans('无权限删除评论！'));
		}
	...
	protected function checkCommentObjectFields($objectType)
	{
		...
		if(!in_array($objectType, $objectTypes)){
			throw $this->createServiceException($this->getServiceKernel()->trans('不存在当前这种评论对象'));
		}
	}
		...
		switch ($comment['objectType']) {
			case self::COMMENT_OBJECTTYPE_COURSE:
				...
				if(empty($foundCourse)){
					throw $this->createServiceException($this->getServiceKernel()->trans('评论课程失败，该课程不存在'));
				}
				break;
