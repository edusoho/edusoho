...
class FileServiceImpl extends BaseService implements FileService {
	...
	public function uploadFile($group, File $file, $target = null) {
		...
		if ($errors) {
			...
			throw $this->createServiceException($this->getServiceKernel()->trans('该文件格式，不允许上传。'));
		}
		$group = $this->getGroupDao()->findGroupByCode($group);
		$user = $this->getCurrentUser();
		if (!$user->isLogin()) {
			throw $this->createServiceException($this->getServiceKernel()->trans('用户尚未登录。'));
		}
		$record = array();
	...
	protected function validateFileIsImage(File $file) {
		...
		if (!$info || empty($info['extension'])) {
			$errors[] = $this->getServiceKernel()->trans('只运行上传JPG、PNG、GIF格式的图片文件。');
		}
	...
	protected function validateFileNameLength(File $file) {
		...
		if (!$file->getFilename()) {
			$errors[] = $this->getServiceKernel()->trans('文件名为空，请给文件取个名吧。');
		}
		if (strlen($file->getFilename()) > 240) {
			$errors[] = $this->getServiceKernel()->trans('文件名超出了240个字符的限制，请重命名后再试。');
		}
		return $errors;
	}
	...
	protected function saveFile($file, $uri) {
		...
		if (!is_writable($directory)) {
			throw $this->createServiceException($this->getServiceKernel()->trans('文件上传路径%directory%不可写，文件上传失败。', array('%directory%' =>$directory )));
		}
	...
	protected function generateUri($group, $file) {
		...
		if (empty($ext)) {
			throw $this->createServiceException($this->getServiceKernel()->trans('获取文件扩展名失败！'));
		}
	...
	public function parseFileUri($uri) {
		...
		if (empty($parts) || count($parts) != 2) {
			throw $this->createServiceException($this->getServiceKernel()->trans('解析文件URI(%uri%)失败！', array('%uri%' =>$uri )));
		}
	...
	public function getImgFileMetaInfo($fileId, $scaledWidth, $scaledHeight) {
		if (empty($fileId)) {
			throw $this->createServiceException($this->getServiceKernel()->trans('参数不正确'));
		}

		$file = $this->getFile($fileId);
		if (empty($file)) {
			throw $this->createServiceException($this->getServiceKernel()->trans('文件不存在'));
		}
