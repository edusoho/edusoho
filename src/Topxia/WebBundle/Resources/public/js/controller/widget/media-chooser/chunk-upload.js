define(function(require, exports, module) {
	...
	var ChunkUpload = Widget.extend({
		...
		uploadLargeFile: function(file){
			...
			var blockCtx = new Array();
			var token = this.getToken();
			for(var i=0; i<blockCount; i++){
				console.log("blockIndex:"+i);
				var blockSize = this.getBlocksize(file.size, i);
				this.uploadBlock(token, file, i, blockSize, blockCtx);
			}
			this.mkFile(token, file, blockCtx);
		},
		...
		uploadBlock: function(token, file, blkIdex, blockSize, blockCtx){
			var firstChunkSize = this.getChunkSize(0, blockSize);
			var firstChunk = this.getChunk(file, blkIdex*this.get("blkSize"), firstChunkSize);
			this.mkChunkWithCrc(token, file, firstChunk, blkIdex, blockSize, blockCtx);
		},
		mkBlock: function(token, file, firstChunk, blkIdex, blockSize, blockCtx, crc){
			var self=this;
			var xhr = new XMLHttpRequest();
            xhr.open('POST', this.get("uploadUrl") + "/mkblk/" + blockSize, false);
                ...
                if (xhr.readyState == 4 && xhr.status == 200 && response != "") {
                    ...
                    if (blkRet) {
                        ...
                        // }else {
                        	...
                        	while(chunkSize>0){
	                        	var chunk = self.getChunk(file, self.get("defaultChunkSize")*chunkIndex, chunkSize);
	                        	blkRet = self.putChunk(chunk,blkRet,token.postParams.token);
	                        	chunkIndex++;
	                        	chunkSize = self.getChunkSize(self.get("defaultChunkSize")*chunkIndex, blockSize);
                        	}
		...
		putChunk: function(chunk, blkRet, token){
			var xhr = new XMLHttpRequest();
			var nextBlkRet;
            xhr.open('POST', this.get("uploadUrl") + "/bput/" + blkRet.ctx + "/" + blkRet.offset, false);
            xhr.setRequestHeader("Authorization", "UpToken " + token);
            xhr.onreadystatechange = function(response) {
				if (xhr.readyState == 4 && xhr.status == 200 && response != "") {
					nextBlkRet=JSON.parse(xhr.responseText);
				}
			};
			xhr.send(chunk);
			return nextBlkRet;
		...
		mkChunkWithCrc: function(token, file, chunk, blkIdex, blockSize, blockCtx) {
	        this.mkBlock(token, file, chunk, blkIdex, blockSize, blockCtx, 'crc');
