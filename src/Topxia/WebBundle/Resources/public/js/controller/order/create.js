		...
		$('button[role="coupon-use"]').click(function(e){
			...
			$.post('/'+data.targetType+'/'+data.targetId+'/coupon/check', data, function(data){
				...
				if(data.useable == "no") {
					...
					$('[role=no-use-coupon-code]').hide();
					$('[role="code-notify"]').removeClass('alert-danger').addClass("alert-success").text(Translator.trans('优惠码可用，您当前使用的是')+((data['type']=='discount')? (Translator.trans('打')+data['rate']+Translator.trans('折')) : (Translator.trans('抵价')+data['rate']+Translator.trans('元')))+Translator.trans('的优惠码'));
