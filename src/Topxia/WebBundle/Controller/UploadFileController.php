...
class UploadFileController extends BaseController
{
    ...
    protected function cloudConvertCallback2(Request $request)
    {
        ...
        $this->getLogService()->info('uploadfile', 'cloud_convert_callback', $this->getServiceKernel()->trans('文件云处理回调'), array('result' => $result));
    ...
    public function cloudConvertCallback3Action(Request $request)
    {
        ...
        $this->getLogService()->info('uploadfile', 'cloud_convert_callback3', $this->getServiceKernel()->trans('文件云处理回调'), array('result' => $result));
        $result = json_decode($result, true);
        $result = array_merge($request->query->all(), $result);
        ...
        if ($result['code'] != 0) {
            $this->getLogService()->error('uploadfile', 'cloud_convert_error', $this->getServiceKernel()->trans('文件云处理失败'), array('result' => $result));
        ...
        if (empty($file)) {
            $this->getLogService()->error('uploadfile', 'cloud_convert_error', $this->getServiceKernel()->trans('文件云处理失败，文件记录不存在'), array('result' => $result));
            $result = array(
                "error" => $this->getServiceKernel()->trans('文件不存在')
            );
    ...
    public function cloudConvertCallbackAction(Request $request)
    {
        ...
        $this->getLogService()->info('uploadfile', 'cloud_convert_callback', $this->getServiceKernel()->trans('文件云处理回调'), array('content' => $data));
