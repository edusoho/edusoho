...
class CourseManageController extends BaseController
{
    ...
    public function priceAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if($price == NULL && $coinSetting['coin_enabled'] ==1 && $coinSetting['course_price_show_type'] == 1){
                $fields['price'] = floatval($coinPrice)*floatval($cashRate);
            }

            if($coinPrice == NULL && $coinSetting['coin_enabled'] ==1 && $coinSetting['course_price_show_type'] ==0){
