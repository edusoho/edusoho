...
class LoginBindController extends BaseController
{
    ...
    public function callbackAction(Request $request, $type)
    {
        ...
        if ($bind) {
            ...
            if (empty($user)) {
                $this->setFlashMessage('danger', $this->getServiceKernel()->trans('绑定的用户不存在，请重新绑定。'));
    ...
    private function autobind(Request $request, $type)
    {
        ...
        if (empty($token)) {
            $response = array('success' => false, 'message' => $this->getServiceKernel()->trans('页面已过期，请重新登录。'));
            goto response;
        }
        ...
        if (empty($oauthUser['id'])) {
            $response = array('success' => false, 'message' => $this->getServiceKernel()->trans('网络超时，获取用户信息失败，请重试。'));
            goto response;
        }

        if (!$this->getAuthService()->isRegisterEnabled()) {
            $response = array('success' => false, 'message' => $this->getServiceKernel()->trans('注册功能未开启，请联系管理员！'));
            goto response;
        }
        ...
        if (empty($user)) {
            $response = array('success' => false, 'message' => $this->getServiceKernel()->trans('登录失败，请重试！'));
            goto response;
        }
    ...
    public function existBindAction(Request $request)
    {
        ...
        if (SimpleValidator::email($data['emailOrMobile'])) {
            ...
            $user    = $this->getUserService()->getUserByVerifiedMobile($data['emailOrMobile']);
            $message = $this->getServiceKernel()->trans('该手机号码尚未注册或绑定');
        }
        ...
        if (empty($user)) {
            ...
            $response = array('success' => false, 'message' => $this->getServiceKernel()->trans('该帐号已经绑定了该第三方网站的其他帐号，如需重新绑定，请先到账户设置中取消绑定！'));
        } else {
            $response = array('success' => true, '_target_path' => $this->getTargetPath($request));
            $this->getUserService()->bindUser($type, $oauthUser['id'], $user['id'], $token);
            $this->authenticateUser($user);
    ...
    public function changeToExistAction(Request $request, $type)
    {
        ...
        if (empty($token)) {
            return $this->createMessageResponse('error', $this->getServiceKernel()->trans('页面已过期，请重新登录。'));
        }
    ...
    protected function mateExistName($type)
    {
        switch ($type) {
            case 'weixinweb':
                return $this->getServiceKernel()->trans('微信绑定已有账号');
                break;
            case 'weixinmob':
                return $this->getServiceKernel()->trans('微信绑定已有账号');
                break;
            case 'weibo':
                return $this->getServiceKernel()->trans('微博绑定已有账号');
                break;
            case 'qq':
                return $this->getServiceKernel()->trans('QQ绑定已有账号');
                break;
            case 'renren':
                return $this->getServiceKernel()->trans('人人绑定已有账号');
                break;
            default:
                return '';
        }
