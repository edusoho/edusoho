...
class EduCloudController extends BaseController
{
    public function smsSendAction(Request $request)
    {
        if ($request->getMethod() == 'POST') {
            if ($this->setting('cloud_sms.sms_enabled') != '1') {
                return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('短信服务被管理员关闭了')));
            }
            ...
            if ($smsType != 'sms_user_pay') {
                ...
                if ($request->getSession()->get('captcha_code') != $captchaNum) {
                    return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('验证码错误')));
                }
            }
            ...
            if (!$this->checkLastTime($smsLastTime, $currentTime, $allowedTime)) {
                return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('请等待120秒再申请'), 'message' => "{$smsLastTime}|{$currentTime}"));
            }
            ...
            if (in_array($smsType, array('sms_bind', 'sms_registration'))) {
                ...
                if ($smsType == 'sms_bind') {
                    $description = $this->getServiceKernel()->trans('手机绑定');
                } else {
                    $description = $this->getServiceKernel()->trans('用户注册');
                }
                ...
                if ($hasVerifiedMobile && ($to == $currentUser['verifiedMobile'])) {
                    return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('您已经绑定了该手机号码')));
                }

                if (!$this->getUserService()->isMobileUnique($to)) {
                    return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('该手机号码已被其他用户绑定')));
                }
            }

            if ($smsType == 'sms_forget_password') {
                $description = $this->getServiceKernel()->trans('登录密码重置');
                $targetUser  = $this->getUserService()->getUserByVerifiedMobile($request->request->get('to'));

                if (empty($targetUser)) {
                    return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('用户不存在')));
                }

                if ((!isset($targetUser['verifiedMobile']) || (strlen($targetUser['verifiedMobile']) == 0))) {
                    return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('用户没有被绑定的手机号')));
                }

                if ($targetUser['verifiedMobile'] != $request->request->get('to')) {
                    return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('手机与用户名不匹配')));
                }
            ...
            if (in_array($smsType, array('sms_user_pay', 'sms_forget_pay_password'))) {
                ...
                if ($smsType == 'sms_user_pay') {
                    $description = $this->getServiceKernel()->trans('网站余额支付');
                } else {
                    $description = $this->getServiceKernel()->trans('支付密码重置');
                }
                ...
                if ($user['verifiedMobile'] != $request->request->get('to')) {
                    return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('您输入的手机号，不是已绑定的手机')));
                }
            ...
            try {
                ...
                return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('发送失败, %message%', 
    ...
    public function smsCallBackAction(Request $request, $targetType, $targetId)
    {
        ...
        if ($originSign != $sign) {
            return $this->createJsonResponse(array('error' => $this->getServiceKernel()->trans('sign不正确')));
        }
    ...
    protected function checkSmsType($smsType, $user)
    {
        if (!in_array($smsType, array('sms_bind', 'sms_user_pay', 'sms_registration', 'sms_forget_password', 'sms_forget_pay_password'))) {
            throw new \RuntimeException($this->getServiceKernel()->trans('不存在的sms Type'));
        }

        if ((!$user->isLogin()) && (in_array($smsType, array('sms_bind', 'sms_user_pay', 'sms_forget_pay_password')))) {
            throw new \RuntimeException($this->getServiceKernel()->trans('用户未登录'));
        }

        if ($this->setting("cloud_sms.{$smsType}") != 'on' && !$this->getUserService()->isMobileRegisterMode()) {
            throw new \RuntimeException($this->getServiceKernel()->trans('该使用场景未开启'));
        }
    }
