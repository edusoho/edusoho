    ...
    public function updateLiveActivity($id, $fields, $activity)
    {
        ...
        if (!$liveActivity['roomCreated']) {
            ...
            if (EdusohoLiveClient::SELF_ES_LIVE_PROVIDER == $liveActivity['liveProvider']) {
                if (EdusohoLiveClient::LIVE_ROOM_PSEUDO == $fields['roomType'] && empty(array_diff($fields['fileIds'], $liveActivity['fileIds']))) {
                    $client = new EdusohoLiveClient();
                    $client->updatePseudoLiveVideo($liveActivity['liveId'], $this->getPseudoLiveVideoUrl($fields));
                } else {
