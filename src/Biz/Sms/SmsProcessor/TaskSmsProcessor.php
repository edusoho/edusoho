...
class TaskSmsProcessor extends BaseSmsProcessor
{
    ...
    public function getSmsParams($targetId, $smsType)
    {
        ...
        $originUrl = $kernel->getContainer()->get('router')->generate('course_task_show', ['courseId' => $task['courseId'], 'id' => $task['id']], $referenceType);
    ...
    public function getSmsUserCount($targetId, $smsType)
    {
        ...
        $courseSet = $this->getCourseSetService()->getCourseSet($task['fromCourseSetId']);
        if (empty($courseSet['parentId'])) {
            return $this->getCourseMemberService()->countMembers(['courseId' => $task['courseId'], 'role' => 'student']);
        }
