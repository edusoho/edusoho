...
use Custom\Service\Course\NextRoundService;
...
class NextRoundServiceImpl extends BaseService implements NextRoundService
{
    ...
    {
        $periodicCount     = $this->getCourseDao()->getPeriodicCoursesCount($course['rootId']);
        $course['periods'] = intval($periodicCount + 1);
        $course['rootId']  = $course['rootId'];

        return $this->copy($course, $link);
    }

    protected function copy($course, $link = false)
    {
    ...
    {
        $fields                = ArrayToolkit::parts($course, array('coinPrice', 'rootId', 'startTime', 'endTime', 'periods', 'originCoinPrice', 'price', 'originPrice', 'title', 'status', 'subtitle', 'type', 'maxStudentNum', 'price', 'coinPrice', 'expiryDay', 'serializeMode', 'lessonNum', 'giveCredit', 'vipLevelId', 'categoryId', 'tags', 'smallPicture', 'middlePicture', 'largePicture', 'about', 'teacherIds', 'goals', 'audiences', 'userId'));
        $fields['createdTime'] = time();
