<?php
namespace Custom\service\Course\Event;
...
class CourseScoreEventSubscriber implements EventSubscriberInterface {
	...
	public function onCourseScoreAdd(ServiceEvent $event) {
		...
		if ($courseScoreSetting['publishType'] == 'auto') {
	...
	private function generateTask($courseScoreSetting) {
		return array(
			...
			'jobClass' => 'Custom\\Service\\Course\\Job\\PublishCourseScore',
	...
	protected function getCrontabJobService() {
		return serviceKernel::instance()->createService("Custom:CrontabJob.CrontabJobService");
	}
}
