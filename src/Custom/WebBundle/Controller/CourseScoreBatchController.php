...
{   
        ...
        foreach ($usersData as $key => $userData) {  
           ...
           if($userScore){
            ...
            if(!empty($userScore['importOtherScore'])){
                if($user)$count['updateUserCount'] ++;
            }else{
                if($user)$count['successCount'] ++;
            }
    ...
    {       
        ...
        if (count($trueNameArray)<=1) return false; 

        $staffNoArray = explode("学号",$data);
        if (count($staffNoArray)<=1) return false; 

        $scoreArray = explode("其他评分",$data);
        if (count($scoreArray)<=1) return false; 
