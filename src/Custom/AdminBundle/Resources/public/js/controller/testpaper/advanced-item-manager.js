define(function(require, exports, module) {
    ...
    var TestpaperItemManager = Widget.extend({
        ...
        refreshTestpaperStats: function() {
            var part = this.get('currentPart');
            var stats = this._calTestpaperStats();
            var html = '试卷总分<strong>' + stats.total.score.toFixed(1) + '</strong>分';
            html += ' <span class="stats-part">';
            if (stats[part].type == 'material') {
                html += stats[part].name + '<strong>' + stats[part].count + '</strong>子题/<strong>' + stats[part].score.toFixed(1) + '</strong>分';
            } else {
                html += stats[part].name + '<strong>' + stats[part].count + '</strong>题/<strong>' + stats[part].score.toFixed(1) + '</strong>分';
            }

            if (stats[part].missScore > 0) {
                html += ' 漏选得分<strong>' + stats[part].missScore + '</strong>分</span>';
            }
        ...
        _calTestpaperStats: function() {
            var stats = {};
            this.$('.testpaper-nav-link').each(function(index) {
                var part = $(this).data('part'),
                    type = $(this).data('type'),
                    saveName = '第' + (index+1) +'部分',
                    name = $(this).data('name');

                stats[part] = {name:name, saveName:saveName, type:type, count:0, score:0, missScore:0};
                $("#testpaper-items-" + part).find('[name="scores[]"][type=text]').each(function() {
                    stats[part]['count'] ++;
                    stats[part]['score'] += parseFloat($(this).val());
                    stats[part]['missScore'] = parseFloat($(this).data('miss-score'));
                });
            });

            var total = {name:'总计', saveName:'总计' ,count:0, score:0};
            $.each(stats, function(index, statsItem) {
        ...
        onClickPickItem: function(e) {
            var $btn = $(e.currentTarget);
            var stats = this._calTestpaperStats();
            var currentPart = this.get('currentPart');
            var excludeIds = [];
            for(var index in stats) {
                if(stats[currentPart].type == stats[index].type) {
                    $("#testpaper-items-" + index).find('[name="questionId[]"]').each(function(){
            ...
            $.get($btn.data('url'), {excludeIds: excludeIds.join(','), type: stats[currentPart].type, part:this.get('currentPart')}, function(html) {
                $modal.html(html);
            });
        },
        ...
        onClickNav: function(e) {
            ...
            $("#testpaper-items-" + $nav.data('part')).removeClass('hide');
            this.set('currentPart', $nav.data('part'));
            this.refreshTestpaperStats();
            return true;
        },
