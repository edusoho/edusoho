      ...
      <div v-if="contracts.length !== 0" v-for="contract in contracts" class="flex justify-between items-center px-16 py-36  border border-[#e4ecf3] border-x-0 border-t-0 border-solid">
        <div class="flex">
          ...
          <div class="flex flex-col">
            <div class="w-320 overflow-hidden text-ellipsis whitespace-nowrap mb-12 text-16 text-[#37393D] font-medium">
              {{contract.name}}
            </div>
            <div class="text-12 text-[#919399] font-normal"><span v-if="contract.relatedGoods.type === 'course'">关联课程：</span><span v-if="contract.relatedGoods.type === 'classroom'">关联班级：</span><span v-if="contract.relatedGoods.type === 'itemBankExercise'">关联题库：</span>{{contract.relatedGoods.name}}</div>
          </div>
        </div>
