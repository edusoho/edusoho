<template>
  <div class="more-mask">
    ...
    <div
      ...
      v-show="exccedHeight"
      :style="textStyle"
      class="more-mask__footer"
      @touchstart.prevent="maskLoadMore"
    >
      {{ text.content || '点击查看更多' }}
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    heightStyle() {
      const maxHeight =
        !this.exccedHeight || this.disabled || this.forceShow
          ? 'none'
          : `${this.maxHeight}px`;
      const paddingBottom =
        this.forceShow || (this.exccedHeight && !this.disabled) ? '25px' : '0';
