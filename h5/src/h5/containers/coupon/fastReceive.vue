<template>
  <div v-if="coupons" class="coupon-receive">
    ...
    <div class="coupon-receive-ticket">
      <div class="ticket-discount">
        <span class="rate">{{ parseFloat(coupons.rate) }}</span
      ...
      <div class="ticket-range text-overflow">
        适用于：{{ couponType(coupons) }}
      </div>
    </div>
    <div
      v-if="hasReceive && !loginMethods && couponSwitch"
    ...
    <div
      v-if="(receiveFail && !loginMethods) || !couponSwitch"
      class="receive-status"
    >
    ...
    <fast-login
      v-if="!login && canuse && couponSwitch"
      @lReceiveCoupon="lReceiveCoupon"
    />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async getCouponInfo() {
      ...
      await Api.getCouponInfo({
        ...
        .then(res => {
          this.coupons = res;
          // 判断优惠券是否还能用
          const cantUseCoupon = !this.cantUseCoupon(res);
          if (cantUseCoupon) {
            this.isLogin();
          }
        })
    ...
    // 领取优惠券后跳转指定页面
    useCoupon() {
      if (this.currentUserCoupon != null && this.currentUserCoupon.deadline) {
        // 已经过期
    ...
    isOld(time) {
      ...
      const d1 = new Date();
      const d2 = new Date(Date.parse(time));
    ...
    cantUseCoupon(coupon) {
      ...
      if (Number(coupon.unreceivedNum) == 0) {
        // 已领完
