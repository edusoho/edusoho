<template>
  <div v-if="coupons" class="coupon-receive">
    ...
    <div v-if="hasReceive && !loginMethods && couponSwitch" class="receive-status">
    ...
    <div v-if="(receiveFail && !loginMethods) || !couponSwitch" class="receive-status">
      <img src="static/images/coupon-no.png" class="status-icon">
      <div class="status-text">{{ failmessage }}</div>
    </div>
    <fast-login v-if="!login && canuse && couponSwitch" @lReceiveCoupon="lReceiveCoupon"/>
  </div>
</template>
<script>
