  ...
  <div class="login-face-verification">
    <div v-if="!errorShow" v-show="regular">
      ...
      <div v-if="!failTextShow" v-show="!tipShow">
        <img :src="imgAddress" class="img-content" alt="人脸照片" />
        <div>{{ verifiedText }}中，请稍候...</div>
      </div>
      <div v-show="failTextShow">
        人脸识别多次{{ verifiedText }}不通过
        <div class="mt5">请改用其它方式{{ verifiedText }}或联系管理员</div>
      </div>
      <div v-show="tipShow">
      ...
      Api.faceSession({
        ...
        .then(res => {
          ...
          if (res.status === 'processing') {
            ...
            const duration = this.requestEndT
              ? this.requestEndT - this.requestStartT
              : 0;
            if (duration > 58000) {
