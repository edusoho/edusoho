<template>
  <div :class="isClassCourse ? '' : 'join-before'">
    ...
    >
      {{
        details.access.code | filterJoinStatus('course', vipAccessToJoin)
      }}</e-footer
    >
    <!-- 秒杀 -->
...
export default {
  ...
  methods: {
    ...mapActions('course', ['joinCourse', 'getAfterCourse']),
      ...
      setTimeout(() => {
        ...
        for (let index = 0; index < tabs.length; index++) {
          const activeCondition =
            this.tops[`${tabs[index]}Top`] - TAB_HEIGHT <= 0;
          if (!activeCondition) {
            continue;
          }
    ...
    activeCurrentTab(scrollTop) {
      ...
      return scrollTop < tops.teacherTop
        ? 0
        : scrollTop >= tops.directoryTop
        ? 2
        : 1;
    },
    handleJoin() {
