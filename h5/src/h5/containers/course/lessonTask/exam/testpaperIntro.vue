  ...
  <div>
    ...
    <div class="intro-body" >
      ...
      <van-panel class="panel intro-panel" title="考试时长">
        <div v-if="limitTime" :class="['intro-panel__content',(result || !disabled) ?'':'intro-tip']">{{ limitTime }}分钟</div>
      ...
      <van-panel class="panel intro-panel" title="题目数量">
        <div class="intro-panel__content">
          <van-cell :border="false" :value="`${sum}题`" class="intro-cell intro-cell--total" title="共计" />
          <van-cell v-for="(item) in question_type_seq" :border="false" :key="item" :title="obj[item]" :value="`${counts[item]}题`" class="intro-cell" />
        </div>
      </van-panel>
    </div>
    <div class="intro-footer">
      <van-button v-if="result" class="intro-footer__btn" type="primary" @click="showResult">查看成绩</van-button>
      <van-button v-else :disabled="disabled" class="intro-footer__btn" type="primary" @click="startTestpaper()">开始考试</van-button>
    </div>
  </div>
</template>
