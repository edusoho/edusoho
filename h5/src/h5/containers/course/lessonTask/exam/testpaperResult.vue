  ...
  <div class="testResults">
    ...
    <div v-if="result" ref="data" class="result-data">
      <div class="result-data__item">
        本次得分
        <div v-if="isReadOver" class="result-data__bottom data-number-orange data-medium"><span class=" data-number">{{ result.score }}</span>分
        </div>
    ...
    <div :style="{height: calHeight}" class="result-subject">
      <van-panel v-for="(keyItem, index) in question_type_seq" :key="index" :title="obj[keyItem]" class="result-panel">
        <ul class="result-list">
          <li
            v-for="(item, index) in subjectList[keyItem]"
            :class="[ 'result-list__item testpaper-number', `circle-${color[item.status]}`]"
            :key="index">{{ item.seq }}
          </li>
        </ul>
      </van-panel>
