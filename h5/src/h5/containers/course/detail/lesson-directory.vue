    ...
    <template v-for="(lessonItem, lessonIndex) in lesson">
      <div v-if="hasLesson" :key="lessonIndex" class="lesson-directory">
        <div
          ...
          "
        >
          <div v-if="lessonItem.tasks[lessonItem.index].videoMaxLevel === '2k'" class="absolute -left-1 -bottom-1 px-8 py-2 text-white text-12 leading-20 font-medium bg-black bg-opacity-80 rounded-tr-12">2K 优享</div>
          <div v-if="lessonItem.tasks[lessonItem.index].videoMaxLevel === '4k'" class="absolute -left-1 -bottom-1 px-8 py-2 text-[#492F0B] text-12 leading-20 font-medium bg-gradient-to-l from-[#F7D27B] to-[#FCEABE] rounded-tr-12">4K 臻享</div>
