<template>
  <div :class="[ brushDo.status === 'doing' ? brushDo.exerciseModes === '0' ? 'ibs-head-top' : 'ibs-one-questions-head' : 'ibs-one-questions-head' ]">
		<div v-if="brushDo.exerciseModes === '1' && brushDo.status === 'doing'" class="w-full">
			<div class="flex justify-between items-end mb-8">
        <span class="text-14 ibs-answer-progress">{{ $t('courseLearning.answerProgress') }} <span class="ibs-current-num">{{ brushDo.reviewedCount }} <span class="ibs-total-number"> /{{ all }}</span></span></span>
        <span @click="itemEngine.cardShow = true" class="text-12 ibs-answer-card">{{ $t('courseLearning.answerSheet') }}</span>
      </div>
      <van-progress :percentage="(brushDo.reviewedCount / all) * 100" stroke-width="8" track-color="#E9E9EB" :show-pivot=false />
		</div>
