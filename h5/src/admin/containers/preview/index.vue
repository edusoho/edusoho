<template>
  <div class="preview-container">
    <div class="image-container clearfix">
      <div
        :class="{ 'phone-img-app': from !== 'miniprogramSetting' }"
        class="phone-img"
      >
        <img :src="bgImg" />
        <div :class="getTitleClass()">
          <i class="iconfont icon-search"></i>{{ settings.name }}
        </div>
        <mobile-preview :class="getClass()" :feedback="false" />
        <find-footer
          :portal="portal"
          class="preview-app-footer"
          v-if="from === 'appSetting'"
        ></find-footer>
      </div>
      <div class="code-container">
        <div v-if="isH5" class="code-item">
          <div class="code-img-container">
            <img :src="qrcode" class="code-image" />
          </div>
          <div class="help-text">
            扫描二维码在手机端预览
            <div>二维码60分钟内首次扫描有效</div>
          </div>
        </div>
        <div v-else class="code-item__img">
          <img class="code-image" src="static/images/preview.png" />
        </div>
        <el-button class="mrs btn-border-primary btn-common" @click="edit"
          >返回编辑</el-button
        >
        <el-button class="btn-common btn-primary" @click="publish"
          >发布</el-button
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...mapActions(['getQrcode', 'saveDraft']),
    edit() {
      this.$router.push({
        name: this.from,
        query: {
