<template>
  <div class="flex flex-col mx-16 mb-12 bg-text-1 text-12" style="border-radius: 6px;" @click="onClick(course.hasCertificate, $event)">
    <div class="flex relative">
      <div :class="errImgUrl ? 'afterBack' : ''"></div>
      <div class="relative" style="width: 170px;height: 100%;border-radius: 6px 0 0 6px;">
        <img class="h-94 course-img" v-lazy="course.imgSrc.url" style="border-radius: 6px 0 0 6px;" />
        ...
        <div v-if="courseType === 'live'" class="absolute bottom-0 w-full text-white font-medium text-12 bg-black bg-opacity-40 text-right flex items-center flex-row-reverse" style="padding: 4px 6px;line-height: 12px; border-bottom-right-radius: 6px; border-bottom-left-radius: 6px;">直播<span class="inline-block w-4 h-4 bg-[#60F1A3] mr-4 text-right" style="border-radius: 9999px;"></span></div>
      </div>
      ...
      if (this.typeList === 'course_list') {
        this.$router.push({
          ...
          query: {
            ...
            type: 'course_list',
            hasCertificate,
            lastLearnTaskId: this.course.bottom.data.lastLearnTask.id,
            lastLearnTaskType: this.course.bottom.data.lastLearnTask.type,
          },
        });
      }
    },
