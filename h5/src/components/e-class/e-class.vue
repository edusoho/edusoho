<template>
  <div class="flex mx-16 mb-12 bg-text-1 text-12 relative" style="height: 94px;border-radius: 6px;" @click="onClick(course.hasCertificate, $event)">
    ...
    <div class="relative" style="width: 170px;height: 100%;border-radius: 6px 0 0 6px;">
      ...
      <div v-if="courseType === 'live'" class="absolute bottom-0 w-full px-6 py-4 text-white font-medium text-12 leading-12 bg-black bg-opacity-40 rounded-bl-6 text-right flex items-center flex-row-reverse">直播<span class="inline-block w-4 h-4 bg-[#60F1A3] mr-4 text-right" style="border-radius: 9999px;"></span></div>
    </div>
