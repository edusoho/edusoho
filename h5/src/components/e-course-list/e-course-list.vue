<template>
  ...
  <div
    v-if="
      (showMode === 'h5' && courseList.items.length) || showMode === 'admin'
    "
    class="e-course-list"
  >
    <div v-if="uiStyle === 'old'" class="e-course-list__header">
      <div class="clearfix">
        <span class="e-course-list__list-title text-overflow">{{
          courseList.title
        }}</span>
...
export default {
  ...
  props: {
    ...
    showMode: {
      // 展示模式  h5、admin 在admin模式下就算列表length为0，也要展示出列表标题。在h5模式下，length为0不展示任何列表标题
      type: String,
      default: 'h5',
    },
    uiStyle: {
  ...
  computed: {
    ...
    platform: {
      get() {
        if (
          this.$route.name === 'appSetting' ||
          this.$route.query.from === 'appSetting'
        ) {
          return 'app';
        }
        return 'h5';
    ...
    displayStyle: {
      // row 表示一行一列   distichout表示一行两列
      get() {
        if (
          this.courseList.displayStyle &&
  ...
  methods: {
    jumpTo(source) {
      ...
      if (this.moreType === 'vip') {
        this.$router.push({
          name:
            this.typeList === 'course_list' ? 'vip_course' : 'vip_classroom',
          query: {
