...
class notemodel
{
    ...
    public function sendone($appid, $noteid = 0, $note = '')
    {
        ...
        $returnsucceed = '' != $response && (1 == $response || is_array(xml_unserialize($response)));
    ...
    public function _gc()
    {
        0 == rand(0, UC_NOTE_GC) && $this->db->query('DELETE FROM '.UC_DBTABLEPRE."notelist WHERE closed='1'");
    }
    ...
    public function _close_note($note, $apps, $returnsucceed, $appid)
    {
        ...
        foreach ($apps as $key => $app) {
            ...
            if (!$app['recvnote'] || 1 == $appstatus || $appstatus <= -UC_NOTE_REPEAT) {
                --$appcount;
            }
        }
        if ($appcount < 1) {
